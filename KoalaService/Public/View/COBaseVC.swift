...
public class COBaseVC: UIViewController,UIAlertViewDelegate,UIActionSheetDelegate {
	...
	func setNavRightBtn(title:String?,imageName:String?,block:(()->Void)? = nil)->UIBarButtonItem{
        ...
        if let _ = title{
            ...
            //Bordered－ios8被弃用－再检查  .Plain
			btnItem = UIBarButtonItem(title: title, style: UIBarButtonItemStyle.Plain, target: self, action: Selector(onRightNavBtnSelectorName))
            ...
            //Bordered－ios8被弃用－再检查  .Plain
			btnItem = UIBarButtonItem(image: UIImage(named: imageName_), style: .Plain, target: self, action: Selector(onRightNavBtnSelectorName))
		}
	...
	func createNavItemBtn(title:String?,imageName:String?,selectorName:String,font:UIFont = UIFont.systemFontOfSize(13))->UIBarButtonItem {
		...
		if let _ = title{
            //Bordered－ios8被弃用－再检查  .Plain
			btnItem = UIBarButtonItem(title: title, style: UIBarButtonItemStyle.Plain, target: self, action: Selector(selectorName))
			btnItem!.setTitleTextAttributes([NSFontAttributeName:font], forState: UIControlState.Normal)
		}else if let imageName_ = imageName {
            //Bordered－ios8被弃用－再检查  .Plain
			btnItem = UIBarButtonItem(image: UIImage(named: imageName_), style: .Plain, target: self, action: Selector(selectorName))
		}
		return btnItem!
	}
	//点击了右边的按钮
