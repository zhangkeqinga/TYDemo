...
class DeployHttpOperation: BaseHttpOperation {
	//获取布防的属性列表(中心负责人专用)
...
//				let resultDict = threadMsg.receiveResultDict!
				...
				let resultDict = threadMsg.receiveResultDict!
				//获取数组,返回
				var resultList = resultDict["orgList"] as! [NSDictionary]
	...
	class func operationGetChildrenOrgDeployInfo(threadModel:COThreadM){
		...
		threadModel.setReceiveToBackgourBlock { (threadMsg) -> Void in
			...
			//获取数组,返回
			var resultList = threadMsg.receiveResultDict!["orgList"] as! [NSDictionary]
